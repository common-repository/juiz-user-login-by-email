...
/*
	...
	Text Domain: juiz-user-login-by-email
...
function make_juiz_ulbe_multilang() {
	load_plugin_textdomain( 'juiz-user-login-by-email', false, JULBE_DIRNAME . '/languages' );
}
...
if ( ! function_exists( 'juiz_add_email_to_login' ) ) {
	...
	function juiz_add_email_to_login( $translated_text, $text, $domain ) {
		...
		if ( 'Username' == $text ) {
			$translated_text .= ' ' . __( 'or Email', 'juiz-user-login-by-email' );
		}
		return $translated_text;
	}
}
